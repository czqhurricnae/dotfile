...
local keyHandler = function(e)
  ...
  if replacement then
    ...
    for k, v in pairs(e:getFlags()) do
      if v and k ~= "alt" then -- alt will be down because that's our "wrapper", so ignore it
...
local modifierHandler = function(e)
  ...
  for k, v in pairs(flags) do
    onlyControlPressed = v and k == "alt"
    if not onlyControlPressed then break end
  end
  -- you must tap and hold alt by itself to turn this on
  if onlyControlPressed and not module.keyListener then
    ...
    -- however, adding additional modifiers afterwards is ok... its only when alt isn't down that we switch back off
  elseif not flags.alt and module.keyListener then
